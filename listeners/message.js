...
client.on("message", async message => {
	...
	const executedBy = getString("executedBy", "global").replace(
		"%%user%%",
		message.author.tag
	);
	...
	if (
		...
		if (
			message.channel.parent.id === "549503328472530977" ||
			message.channel.parent.id === "748585307825242322" ||
			message.channel.parent.id === "763131996163407902" ||
			message.channel.parent.id === "646083561769926668"
		) {
			//Hypixel, SkyblockAddons, Bot and Quickplay Translations
			const langFix = message.content
				.replace(/translate\.hypixel\.net/gi, "crowdin.com")
				.replace(/\/en-(?!en#)[a-z]{2,4}/gi, "/en-en");
			if (/\/en(-?[a-z]{2,4})?[^#-]/gi.test(message.content)) {
				...
				const embed = new Discord.MessageEmbed()
					...
					.setDescription(
						getString("example", "global").replace(
							"%%url%%",
							"https://crowdin.com/translate/hypixel/286/en-en#106644"
						)
					)
					.setImage("https://i.imgur.com/eDZ8u9f.png");
				if (
					message.content !== langFix &&
					message.channel.parent.id === "549503328472530977"
				)
					embed.setDescription(
						`${getString("example", "global").replace(
							"%%url%%",
							"<https://crowdin.com/translate/hypixel/286/en-en#106644>"
						)}\n${getString("reminderLang", "global").replace(
							"%%format%%",
							"`crowdin.com/translate/hypixel/.../en-en#`"
						)}`
					);
			...
			if (
				...
				const embed = new Discord.MessageEmbed()
					...
					.setAuthor(getString("errors.wrongLink", "global"))
					.setTitle(
						getString("linkCorrectionDesc", "global").replace(
							"%%format%%",
							"`crowdin.com/translate/hypixel/.../en-en#`"
						)
					)
	...
	if (
		/(https:\/\/)([a-z]{2,}\.)?crowdin\.com\/profile\/\S{1,}/gi.test(
			message.content
		)
	)
	...
	if (message.channel.type === "dm")
		console.log(
			message.author.tag + " used command " + commandName + " in DMs"
		);

	//Return if user is not verified
	if (
		message.member &&
		!message.member.roles.cache.has("569194996964786178") &&
		command.name !== "verify"
	)
		return;
	//Verified
		...
		if (
			!user.roles.cache.has("569194996964786178") &&
			command.name !== "verify"
		)
			return; //Verified
	}
	...
	if (message.guild?.id === "549503328472530974") {
		...
		if (
			...
			command.categoryBlacklist.includes(message.channel.parent.id)
		)
			allowed = false;
		else if (
			command.channelBlacklist &&
			command.channelBlacklist.includes(message.channel.id)
		)
			allowed = false;
		else if (
			command.categoryWhitelist &&
			!command.categoryWhitelist.includes(message.channel.parent.id)
		)
			allowed = false;
		else if (
			command.channelWhitelist &&
			!command.channelWhitelist.includes(message.channel.id)
		)
			allowed = false;

		//Prevent users from running commands in development
		if (command.dev && !message.member.roles.cache.has("764442984119795732"))
			allowed = false; //Discord Administrato

		//Give perm to admins and return if not allowed
		if (message.member.hasPermission("MANAGE_ROLES") && command.name !== "eval")
			allowed = true;
	} else allowed = false;
	if (!allowed) {
		message.react("732298639736570007");
	...
	if (timestamps.has(message.author.id)) {
		...
		if (now < expirationTime) {
			...
			if (Math.ceil(timeLeft) >= 120) {
				timeLeftS = getString("minsLeftT", "global")
					.replace("%%time%%", Math.ceil(timeLeft / 60))
					.replace("%%command%%", commandName);
			} else if (Math.ceil(timeLeft) === 1) {
				timeLeftS = getString("secondLeft", "global").replace(
					"%%command%%",
					commandName
				);
			} else {
				timeLeftS = getString("timeLeftT", "global")
					.replace("%%time%%", Math.ceil(timeLeft))
					.replace("%%command%%", commandName);
			}
			const embed = new Discord.MessageEmbed()
				.setColor(errorColor)
	...
	function getString(path, cmd, lang) {
		...
		try {
			strings = require(`../strings/${lang || author.lang}/${
				cmd || command.name
			}.json`);
		} catch {
			strings = require(`../strings/en/${cmd || command.name}.json`);
		}
		...
		pathSplit.forEach(pathPart => {
			if (pathPart) {
				...
				if (
					typeof jsonElement === "object" &&
					pathSplit.indexOf(pathPart) !== pathSplit.length - 1
				) {
					...
					if (!string) {
						...
						if (!string) {
							...
							console.error(
								`Couldn't get string ${path} in English for ${
									cmd || command.name
								}, please fix this`
							);
						}
	...
	try {
		...
		if (getString(command.name, "help") === "strings.dm.usage")
			embed.addFields({
				name: getString("usage", "global"),
				value: `\`${command.usage}\``
			});
		else
			embed.addFields({
				name: getString("usage", "global"),
				value: `\`${getString(`${command.name}.usage`, "help")}\``
			});
		...
		return message.channel.send(embed).then(msg => {
			if (error.stack && process.env.NODE_ENV === "production") {
				...
				message.guild.channels.cache
					.get("730042612647723058")
					.send("ERROR INCOMING, PLEASE FIX <@240875059953139714>", embed); //Rodry and bot-development
				console.error(
					`Unexpected error with command ${commandName} on channel ${
						message.channel.name || message.channel.type
					} executed by ${message.author.tag}. Here's the error:\n${
						error.stack
					}`
				);
			} else {
				setTimeout(() => {
					if (!message.deleted && message.channel.type !== "dm")
						message.delete();
		...
		if (command.allowTip !== false && d < 0.05) {
			...
			message.channel.send(
				`**${getString("tip", "global").toUpperCase()}:** ${tip
					.replace("%%botUpdates%%", "<#732587569744838777>")
					.replace("%%gettingStarted%%", "<#699275092026458122>")
					.replace("%%twitter%%", "<https://twitter.com/HTranslators>")
					.replace("%%rules%%", "<#796159719617986610>")
					.replace("%%serverInfo%%", "<#762341271611506708>")
					.replace("%%bots%%", "<#549894938712866816>")}`
			);
		}
	}
});
