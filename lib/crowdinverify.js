...
async function crowdinVerify(message) {
	...
	const evalReturn = await page.evaluate(message => {
		if (document.querySelector(".user-about")?.textContent.includes(message)) {
			const projects = window.eval(
				crowdin.profile_projects.view.state.projects
			);
			if (projects) return projects;
			else return { error: true, type: "private-profile" };
		} else if (document.querySelector(".error-page") !== null)
			return { error: true, type: "error-page" };
		else return;
	}, message.author.tag);
	await page.close();
	...
	if (!evalReturn) {
		...
		return message.author.send(embed)
			.then(() =>
				message.client.channels.cache
					.get(UsefulIDs.logChannel).send(`${message.author} forgot to add their Discord to their profile. Let's hope they fix that with the message I just sent them.`)
			)
			.catch(() => {
		...
		if (evalReturn.type === "private-profile") {
			errorEmbed
		...
		} else { //if profile leads to a 404 page
			errorEmbed
				.setDescription("Hey there! We noticed you tried to send us your Crowdin profile but the link you sent was invalid. This may have happened because you either typed the wrong name in the link or you sent us the generic Crowdin profile link. If you don't know how to obtain the profile URL, make sure it follows the format `https://crowdin.com/profile/<username>` and replace <username> with your username like shown below.\n\nIf you have any questions, be sure to send them to us!")
				.setImage("https://i.imgur.com/7FVOSfT.png")
			return message.author.send(errorEmbed)
	...
	message.author.send(dmEmbed)
		...
		.catch(() => {
			...
			message.client.channels.cache.get(UsefulIDs.logChannel).send(logEmbed);
		});
}
