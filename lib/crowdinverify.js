const puppeteer = require("puppeteer"),
	...
	{ neutralColor } = require("./../config.json");
...
async function crowdinVerify(message) {
	...
	if (Object.keys(endingMessageProjects).length)
		for (const [k, v] of Object.entries(endingMessageProjects)) {
			embed.addField(k, v.join(",\n"));
		}

	message.client.channels.cache.get(UsefulIDs.logChannel).send(embed);
	embed
		.setColor(neutralColor)
		.setAuthor("Received message from staff")
		.setDescription(
			`Hey there!\nYou have successfully verified your Crowdin account! ${
				Object.keys(endingMessageProjects).length
					? "You have received a few roles in our Discord server."
					: "Sadly you didn't receive any roles because you haven't translated for any of the projects.\nWhen you have started translating you can refresh your roles by running `"
			}`
		)
		.setImage("https://i.imgur.com/BM2bJ4W.png")
		.setFooter("Any messages you send here will be sent to staff.");
	message.author
		...
		.then(() => {})
		.catch(() => {});
	//TODO normal embed if DMs closed. Edit logChannel embed accordingly
}
