...
const projectIDs = {
	...
	UsefulIDs = {
		...
		AnnouncementChannel: "662660931838410754"
	};
...
async function crowdinVerify(message) {
	...
	updateLanguageRoles(highestLangRoles);
	...
	await getDb()
		.collection("users")
		.updateOne({ id: message.member.user.id }, { $set: { profile: args[4] } });

	message.client.channels.cache
		.get(UsefulIDs.AnnouncementChannel)
		.send(
...
async function updateProjectRoles(projectName, message, project) {
	...
	const projectTransRole = await message.guild.roles.cache.find(
			r => r.name === `${projectName} Translator`
		),
		projectProofRole = await message.guild.roles.cache.find(
			r => r.name === `${projectName} Proofreader`
		),
		projectManagerRole = await message.guild.roles.cache.find(
			r => r.name === `${projectName} Manager`
		);

	if (highestRole === "Translator") {
		if (await message.member.roles.cache.has(projectProofRole))
			await message.member.roles.remove(projectProofRole);

		if (await message.member.roles.cache.has(projectManagerRole))
			await message.member.roles.remove(projectManagerRole);

		await message.member.roles.add(projectTransRole);
	} else if (highestRole === "Proofreader") {
		if (await message.member.roles.cache.has(projectTransRole))
			await message.member.roles.remove(projectTransRole);

		if (await message.member.roles.cache.has(projectManagerRole))
			await message.member.roles.remove(projectManagerRole);

		await message.member.roles.add(projectProofRole);
	} else {
		if (await message.member.roles.cache.has(projectTransRole))
			await message.member.roles.remove(projectTransRole);
		...
		await message.member.roles.add(projectManagerRole);
	}
}
 ...
 */
async function updateLanguageRoles(highestLangRoles) {
	const coll = await getDb().collection("langdb"),
		activeRoles = [],
		addedRoles = [];

	for (const [k, v] of Object.entries(highestLangRoles)) {
		activeRoles.push(`${coll.findOne({ code: k }).name} ${v}`);
	}
	...
	activeRoles
		...
		.forEach(async p => {
			await message.member.roles.add(
				await message.guild.roles.cache.find(r => r.name === p)
			);
		});

	addedRoles
		.filter(p => !p.includes(activeRoles.filter(pj => b.includes(pj))))
		.forEach(async d => {
			await message.member.roles.remove(
				await message.guild.roles.cache.find(r => r.name === d)
			);
		});
}
