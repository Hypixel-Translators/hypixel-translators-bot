...
async function crowdinVerify(message) {
    ...
    if (!url) {
        ...
        url = userDb.profile
        if (!url) { //if user runs +unverify and the profile is not stored on our DB
            errorEmbed
    ...
    try {
        ...
        if (!eval) {
            ...
            return message.author
                ...
                .catch(() => {
                    errorEmbed.setFooter("")
                    message.client.channels.cache.get(UsefulIDs.verifyChannel).send(message.author, errorEmbed)
                    message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} sent the wrong profile link. Let’s hope they work their way around with the message I just sent in <#${UsefulIDs.verifyChannel}> since they had DMs off.`)
                })
        } else {
            errorEmbed
