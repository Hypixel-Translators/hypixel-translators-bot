...
async function crowdinVerify(message) {
    ...
    if (!url) {
        ...
        if (!url) { //if user runs +reverify and the profile is not stored on our DB
            ...
            return message.author.send(errorEmbed)
                .then(() => {
                    message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} sent the wrong profile link. Let’s hope they work their way around with the message I just sent them.`)
                })
                .catch(() => {
                    errorEmbed.setFooter("")
                    message.client.channels.cache.get(UsefulIDs.verifyChannel).send(`${message.author} you had DMs disabled, so here's our message,`, errorEmbed)
                        .then(msg => {
                            setTimeout(() => {
                                if (!msg.deleted) msg.delete()
                            }, 30000)
                        })
                    message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} sent the wrong profile link. Let’s hope they work their way around with the message I just sent in <#${UsefulIDs.verifyChannel}> since they had DMs off.`)
                })
        }
    }
    ...
    try {
        ...
        if (!eval) { //if profile leads to a 404 page
            ...
            if (!message.deleted) message.delete()
            return message.author.send(errorEmbed)
                .then(() => message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} sent the wrong profile link. Let’s hope they work their way around with the message I just sent them.`))
                .catch(() => {
                    errorEmbed.setFooter("")
                    message.client.channels.cache.get(UsefulIDs.verifyChannel).send(`${message.author} you had DMs disabled, so here's our message,`, errorEmbed)
                        .then(msg => {
            ...
            if (!message.deleted) message.delete()
            return message.author.send(errorEmbed)
                .then(() => message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author}'s profile was private, I let them know about that.`))
                .catch(() => {
                    errorEmbed.setFooter("")
                    message.client.channels.cache.get(UsefulIDs.verifyChannel).send(`${message.author} you had DMs disabled, so here's our message,`, errorEmbed)
                        .then(msg => {
                    ...
                    message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author}'s profile was private, I let them know about that in <#${UsefulIDs.verifyChannel}> since they had DMs off.`)
                })
        }
    }
    const evalReturn = await page.evaluate(tag => {
        if (document.querySelector(".user-about")?.textContent.includes(tag)) return window.eval(crowdin.profile_projects.view.state.projects)
        else return
    }, message.author.tag)
    await page.close()
    closeConnection(browser.uuid)
    ...
    if (!evalReturn) {
        ...
        return message.author.send(embed)
            .then(() => message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} forgot to add their Discord to their profile. Let's hope they fix that with the message I just sent them.`))
            .catch(() => {
                ...
                message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author} forgot to add their Discord to their profile. Let's hope they fix that with the message I just sent them.`)
            })
    }
    ...
    await getDb().collection("users").updateOne({ id: message.member.user.id }, { $set: { profile: url } })
    ...
    message.author.send(dmEmbed)
        .then(() => message.client.channels.cache.get(UsefulIDs.logChannel).send(logEmbed))
        .catch(() => {
