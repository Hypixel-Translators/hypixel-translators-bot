...
async function crowdinVerify(message) {
    ...
    try {
        ...
        const eval = await page.evaluate(() => {
            return document.querySelector(".private-profile") !== null
        })
        await page.close()
        ...
        if (!eval) {
            ...
            return message.author.send(errorEmbed)
                .then(() =>
                    message.client.channels.cache.get(UsefulIDs.logChannel).send(`${message.author}'s profile was private, I let them know about that.`)
                )
                .catch(() => {
                    errorEmbed.setFooter("")
                    message.client.channels.cache.get(UsefulIDs.verifyChannel).send(`${message.author} you had DMs disabled, so here's our message,`, errorEmbed)
    ...
    const evalReturn = await page.evaluate((message) => {
        if (document.querySelector(".user-about")?.textContent.includes(message)) return window.eval(crowdin.profile_projects.view.state.projects)
        else return
    }, message.author.tag)
    await page.close()
    closeConnection(browser.uuid)
...
async function updateLanguageRoles(highestLangRoles, message) {
    ...
    addedRoles
        ...
        .forEach(async d => {
            await message.member.roles.remove(message.guild.roles.cache.find(r => r.name === d).id, "User no longer has this role on Crowdin")
        })
}
