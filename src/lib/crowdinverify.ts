...
async function crowdinVerify(message: Discord.Message) {
    ...
    message.author
        ...
        .then(() =>
            (message.client.channels.cache.get(
                UsefulIDs.logChannel
            ) as Discord.TextChannel).send(logEmbed)
        )
        .catch(() => {
            logEmbed.setFooter("Message not sent because user had DMs off")
            const logMessage = message.client.channels.cache.get(
                UsefulIDs.logChannel
            ) as Discord.TextChannel
...
async function updateProjectRoles(
    ...
    const projectTransRole = message.guild!.roles.cache.find(
            (r) => r.name === `${projectName} Translator`
        )!.id,
        projectProofRole = message.guild!.roles.cache.find(
...
async function updateLanguageRoles(
    ...
    activeRoles
        ...
        .forEach(async (p) => {
            await message.member!.roles.add(
                message.guild!.roles.cache.find((r) => r.name === p)!.id,
                "User has received this role on Crowdin"
            )
        })
