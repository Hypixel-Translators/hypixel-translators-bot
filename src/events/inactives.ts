...
export default async function check() {
	...
	client.guilds.cache.get(ids.guilds.main)!.roles.cache.get(ids.guilds.main)!.members
		...
		.forEach(async member => { //Get all members from the cache
			...
			if (member.roles.cache.has(ids.roles.alerted)) {
				if (Number(userDb.unverifiedTimestamp) <= verify || member.joinedTimestamp! <= verify) {
					...
					await member.send("You stood in the verify channel for too long and, because of that, you have been automatically verified as a player. If you're a translator and wish to receive your roles, please execute `/verify <profileURL>` replacing <profileURL> with the URL to your Crowdin profile.")
						.then(async () => {
