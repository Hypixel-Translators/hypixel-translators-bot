...
export default async function handleAutocompleteInteractions(interaction: AutocompleteInteraction) {
    ...
    if (name === "language" && typeof value === "string") {
        ...
        if (language) return await interaction.respond([{ name: language.name, value: language.code }])
        ...
        return await interaction.respond(results.map(l => ({ name: l.name, value: l.code })))
    } else if (name === "case") {
