    ...
    execute(interaction: Discord.CommandInteraction, args: string[], getString: (path: string, variables?: { [key: string]: string | number } | string, cmd?: string, lang?: string) => any) {
        ...
        if (interaction.guild?.id === "549503328472530974") {
            let joinedHypixel,
                ...
                joinedSba,
                joinedBot
            if (interaction.member!.roles.cache.find(role => role.name === "Hypixel Translator" || role.name === "Hypixel Proofreader" || role.name === "Hypixel Manager")) joinedHypixel = `<:vote_yes:732298639749152769> **${getString("alreadyJoined")}**`
            else joinedHypixel = `<:vote_no:732298639736570007> **${getString("notJoined")}**`
            if (interaction.member!.roles.cache.find(role => role.name === "Quickplay Translator" || role.name === "Quickplay Proofreader" || role.name === "Quickplay Manager")) joinedQuickplay = `<:vote_yes:732298639749152769> **${getString("alreadyJoined")}**`
            else joinedQuickplay = `<:vote_no:732298639736570007> **${getString("notJoined")}**`
            if (interaction.member!.roles.cache.find(role => role.name === "SkyblockAddons Translator" || role.name === "SkyblockAddons Proofreader" || role.name === "SkyblockAddons Manager")) joinedSba = `<:vote_yes:732298639749152769> **${getString("alreadyJoined")}**`
            else joinedSba = `<:vote_no:732298639736570007> **${getString("notJoined")}**`
            if (interaction.member!.roles.cache.find(role => role.name === "Bot Translator" || role.name === "Bot Proofreader" || role.name === "Bot Manager")) joinedBot = `<:vote_yes:732298639749152769> **${getString("alreadyJoined")}**`
