...
const command: Command = {
	...
	async execute(interaction) {
		...
		if (startDay > 31 || endDay > 31) return void (await interaction.reply({ content: "You must input a valid day!", ephemeral: true }))
		else if (startYear < new Date().getFullYear() || startYear > new Date().getFullYear() + 1)
			return void (await interaction.reply({ content: "You must input a valid year!", ephemeral: true }))
		else if (endDate.getTime() < startDate.getTime())
			return void (await interaction.reply({ content: "The ending date must be after the starting date!", ephemeral: true }))
		else if (endDate.getTime() <= yesterday.getTime() || startDate.getTime() <= yesterday.getTime())
			return void (await interaction.reply({ content: "The end and start date must both be after yesterday!", ephemeral: true }))
