...
module.exports = {
  ...
  execute(strings, message) {
    //message.delete();
    message.channel.send(strings.pong);
    message.edit(strings.message.replace("%%pingEmote%%", "<:ping:620954198493888512>").replace("%%ping%%", `${Date.now() - message.createdTimestamp}`).replace("%%latency%%", `${Math.round(message.client.ws.ping)}ms`));
}
};
